...
if (! class_exists('TGM_Plugin_Activation')) {
	...
	class TGM_Plugin_Activation
	{
		 ...
		 */
		public const TGMPA_VERSION = '2.6.1';
		 ...
		 * @since 2.5.0
		 */
		public const WP_REPO_REGEX = '|^http[s]?://wordpress\.org/(?:extend/)?plugins/|';
		 ...
		 * @since 2.5.0
		 */
		public const IS_URL_REGEX = '|^http[s]?://|';
		...
		public function init(): void
		{
			 ...
			 */
			if (apply_filters('tgmpa_load', (is_admin() && ! defined('DOING_AJAX'))) !== true) {
				return;
			}
			...
			if ($this->is_tgmpa_complete() !== true) {
				// Sort the plugins.
			...
			if ($this->has_forced_activation === true) {
				add_action('admin_init', [ $this, 'force_activation' ]);
			}

			// Setup the force deactivation hook.
			if ($this->has_forced_deactivation === true) {
		...
		public function load_textdomain(): void
		{
			...
			if (strpos(__FILE__, WP_PLUGIN_DIR) !== false || strpos(__FILE__, WPMU_PLUGIN_DIR) !== false) {
		...
		public function overload_textdomain_mofile($mofile, $domain)
		{
			...
			if ($domain !== 'tgmpa' || strpos($mofile, WP_LANG_DIR) === false || @is_readable($mofile)) {
				return $mofile;
			}
		...
		public function admin_init()
		{
			...
			if (isset($_REQUEST['tab']) && $_REQUEST['tab'] === 'plugin-information') {
		 ...
		 */
		protected function add_admin_menu(array $args): void
		{
			if (has_filter('tgmpa_admin_menu_use_add_theme_page')) {
				_deprecated_function('The "tgmpa_admin_menu_use_add_theme_page" filter', '2.5.0', esc_html__('Set the parent_slug config variable instead.', 'tgmpa'));
			}

			if ($this->parent_slug === 'themes.php') {
		...
		public function install_plugins_page()
		{
			...
			if ((($plugin_table->current_action() === 'tgmpa-bulk-install' || $plugin_table->current_action() === 'tgmpa-bulk-update') && $plugin_table->process_bulk_actions()) || $this->do_plugin_install()) {
				return;
			}
		...
		protected function do_plugin_install()
		{
			...
			if ((isset($_GET['tgmpa-install']) && $_GET['tgmpa-install'] === 'install-plugin') || (isset($_GET['tgmpa-update']) && $_GET['tgmpa-update'] === 'update-plugin')) {
				...
				$api           = ($this->plugins[ $slug ]['source_type'] === 'repo') ? $this->get_plugins_api($slug) : null;
				$api           = ($api !== false) ? $api : null;
				...
				$title     = ($install_type === 'update') ? $this->strings['updating'] : $this->strings['installing'];
				$skin_args = [
					'type'   => ($this->plugins[ $slug ]['source_type'] !== 'bundled') ? 'web' : 'upload',
				...
				if ($this->is_automatic && ! $this->is_plugin_active($slug)) {
					...
					if ($this->activate_single_plugin($plugin_activate, $slug, true) === false) {
				...
				if ($this->activate_single_plugin($this->plugins[ $slug ]['file_path'], $slug) === false) {
					return true; // Finish execution of the function early as we encountered an error.
				}
			}
		...
		public function maybe_adjust_source_dir($source, $remote_source, $upgrader)
		{
			...
			if (count($source_files) === 1 && $GLOBALS['wp_filesystem']->is_dir($source) === false) {
				return $source;
			}
			...
			if ($upgrader->bulk === false && ! empty($upgrader->skin->options['extra']['slug'])) {
			...
			if (! empty($desired_slug)) {
				...
				if (! empty($subdir_name) && $subdir_name !== $desired_slug) {
					...
					if ($GLOBALS['wp_filesystem']->move($from_path, $to_path) === true) {
		...
		public function notices()
		{
			...
			foreach ($this->plugins as $slug => $plugin) {
				if ($this->is_plugin_active($slug) && $this->does_plugin_have_update($slug) === false) {
					continue;
				}
				...
				if (! $this->is_plugin_installed($slug)) {
					...
					if ($this->does_plugin_require_update($slug) || $this->does_plugin_have_update($slug) !== false) {
						...
						if (current_user_can('update_plugins')) {
							...
							} elseif ($this->does_plugin_have_update($slug) !== false) {
		 ...
		 * @since 2.5.0
		 */
		protected function display_settings_errors(): void
		{
		...
		public function register($plugin)
		{
			...
			if ($plugin['force_deactivation'] === true) {
		...
		public function get_info_link($slug)
		{
			...
			} elseif ($this->plugins[ $slug ]['source_type'] === 'repo') {
				$url = add_query_arg(
					[
						'tab'       => 'plugin-information',
						'plugin'    => urlencode($slug),
		...
		protected function is_core_update_page()
		{
			...
			if ($screen->base === 'update-core') {
				// Core update screen.
				return true;
			} elseif ($screen->base === 'plugins' && ! empty($_POST['action'])) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
				// Plugins bulk update screen.
				return true;
			}

			return (bool) ($screen->base === 'update' && ! empty($_POST['action']))  // phpcs:ignore WordPress.Security.NonceVerification.Missing
		...
		public function can_plugin_update($slug)
		{
			...
			if ($this->plugins[ $slug ]['source_type'] !== 'repo') {
				return true;
			}

			$api = $this->get_plugins_api($slug);

			if ($api !== false && isset($api->requires)) {
		...
		public function is_plugin_updatetable($slug)
		{
			...
			return ($this->does_plugin_have_update($slug) !== false && $this->can_plugin_update($slug));
		}
		...
		public function force_deactivation(): void
		{
			...
			foreach ($this->plugins as $slug => $plugin) {
				 ...
				 */
				if ($plugin['force_deactivation'] === true && is_plugin_active($plugin['file_path'])) {
	...
	class TGMPA_List_Table extends WP_List_Table
	{
		...
		protected function categorize_plugins_to_views()
		{
			...
			foreach ($this->tgmpa->plugins as $slug => $plugin) {
				if ($this->tgmpa->is_plugin_active($slug) && $this->tgmpa->does_plugin_have_update($slug) === false) {
				...
				if (! $this->tgmpa->is_plugin_installed($slug)) {
					...
					if ($this->tgmpa->does_plugin_have_update($slug) !== false) {
		 ...
		 */
		protected function set_view_totals($plugins): void
		{
		...
		protected function get_plugin_status_text($slug)
		{
			...
			if ($this->tgmpa->does_plugin_require_update($slug) && $this->tgmpa->does_plugin_have_update($slug) === false) {
				$update_status = __('Required Update not Available', 'tgmpa');
			} elseif ($this->tgmpa->does_plugin_require_update($slug)) {
				$update_status = __('Requires Update', 'tgmpa');
			} elseif ($this->tgmpa->does_plugin_have_update($slug) !== false) {
		...
		public function get_columns()
		{
			...
			if ($this->view_context === 'all' || $this->view_context === 'update') {
		...
		protected function get_row_actions($item)
		{
			...
			if (! $this->tgmpa->is_plugin_installed($item['slug'])) {
				...
				if ($this->tgmpa->does_plugin_have_update($item['slug']) !== false && $this->tgmpa->can_plugin_update($item['slug'])) {
					/* translators: %2$s: plugin name in screen reader markup */
		...
		public function get_bulk_actions()
		{
			...
			if ($this->view_context !== 'update' && $this->view_context !== 'activate') {
				if (current_user_can('install_plugins')) {
					$actions['tgmpa-bulk-install'] = __('Install', 'tgmpa');
				}
			}

			if ($this->view_context !== 'install') {
				if (current_user_can('update_plugins')) {
		...
		public function process_bulk_actions()
		{
			...
			if ($this->current_action() === 'tgmpa-bulk-install' || $this->current_action() === 'tgmpa-bulk-update') {

				check_admin_referer('bulk-' . $this->_args['plural']);

				$install_type = 'install';

				if ($this->current_action() === 'tgmpa-bulk-update') {
					$install_type = 'update';
				}
				...
				foreach ($plugins_to_install as $key => $slug) {
					...
					if ($install_type === 'install' && $this->tgmpa->is_plugin_installed($slug) === true) {
						unset($plugins_to_install[ $key ]);
					}

					// For updates: make sure this is a plugin we *can* update (update available and WP version ok).
					if ($install_type === 'update' && $this->tgmpa->is_plugin_updatetable($slug) === false) {
						unset($plugins_to_install[ $key ]);
					}
				}
			...
			if ($this->current_action() === 'tgmpa-bulk-activate') {
				check_admin_referer('bulk-' . $this->_args['plural']);
		...
		if (isset($_GET['page']) && $tgmpa_instance->menu === $_GET['page']) {
			...
			if (! class_exists('TGMPA_Bulk_Installer')) {
				...
				class TGMPA_Bulk_Installer extends Plugin_Upgrader
				{
					...
					public function __construct($skin = null)
					{
						...
						if (isset($this->skin->options['install_type']) && $this->skin->options['install_type'] === 'update') {
					...
					public function run($options)
					{
						...
						if ($this->tgmpa->is_automatic) {
							if ($this->skin->options['install_type'] === 'update') {
			...
			if (! class_exists('TGMPA_Bulk_Installer_Skin')) {
				...
				class TGMPA_Bulk_Installer_Skin extends Bulk_Upgrader_Skin
				{
					...
					{
						if ($this->options['install_type'] === 'update') {
							parent::add_strings();
